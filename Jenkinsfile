...
pipeline {
  ...
  environment {
    ...
    GIT_BRANCH      = 'master'
    EMAIL           = '''saurabh_srivastava41@optum.com, kshitij_bisen@optum.com'''
    ...
    stage('2. Source Control Mgmt') {
        ...
        steps { echo '''#########################################'''
            ...
            script {
                try {  
                    dir ("$PROJECT") {
                        git branch: "${env.GIT_BRANCH}", credentialsId: 'f9ec78c4-5932-4c26-9288-a96fe695aaa1', url: 'https://github.optum.com/OptumRxAutomation/Samplemaven.git'
                    }
                }
                catch (exc) {
